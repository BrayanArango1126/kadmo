...
public class DatoUsuarioController {
	...
	public ResponseEntity<DatosUsuariosDTO> getDatoByIdUsuario(@RequestBody UsuariosDTO usuario) {
		...
		return new ResponseEntity<DatosUsuariosDTO>(datoUsuario, HttpStatus.ACCEPTED);
	}

	@PostMapping(value = "add-dato-usuario", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ApiResponse<DatosUsuariosDTO>> postDatoUsuario(
			@Valid @RequestBody DatosUsuariosDTO newDatoUsuario, BindingResult bindingResult) {
		...
		return new ResponseEntity<>(new ApiResponse<>("Datos de usuario agregados correctamente", addedDatoUser),
				HttpStatus.CREATED);
	}

	@PutMapping(value = "update-dato-usuario", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ApiResponse<DatosUsuariosDTO>> putDatoUsuario(
			@Valid @RequestBody DatosUsuariosDTO updateDatoUsuario, BindingResult bindingResult) {
		if (bindingResult.hasErrors()) {
			return new ResponseEntity<>(new ApiResponse<>(bindingResult.getFieldError().getDefaultMessage(), null),
					HttpStatus.CONFLICT);
		}
		...
		return new ResponseEntity<>(new ApiResponse<>("Datos de usuario editados correctamente", updatedDatoUser),
				HttpStatus.OK);
	}
